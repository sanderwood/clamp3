...
## Overview
CLaMP 3 is a system designed to enhance music information retrieval (MIR) across various musical modalities and languages. Using contrastive learning, it aligns sheet music, audio, and multilingual text into a shared representation space, achieving top performance on MIR tasks. Below is a description of the scripts in the [code/](https://github.com/sanderwood/clamp3/tree/main/code) folder.
...
This script contains the training hyperparameters and file paths for the [train_clamp3_audio.py](https://github.com/sanderwood/clamp3/blob/main/code/train_clamp3_audio.py), [train_clamp3_symbolic.py](https://github.com/sanderwood/clamp3/blob/main/code/train_clamp3_symbolic.py), and [train_m3.py](https://github.com/sanderwood/clamp3/blob/main/code/train_m3.py) scripts. You can adjust parameters like learning rates, batch sizes, and data file locations for training.
...
For better performance with symbolic music data, the **C2 variant** of CLaMP 3 is recommended. However, note that its performance on audio data is less optimal. To switch to the C2 variant, modify line 66 of [config.py](https://github.com/sanderwood/clamp3/blob/main/code/config.py), changing the setting from **SAAS** to **C2**.
...
### 2. [extract_clamp3.py](https://github.com/sanderwood/clamp3/blob/main/code/extract_clamp3.py)
...
For data preprocessing, use the scripts in the [preprocessing/](https://github.com/sanderwood/clamp3/tree/main/preprocessing) folder:
- **Text files** (.txt) are processed directly.
- **Sheet music** (.abc) is preprocessed into interleaved ABC notation using scripts in [preprocessing/abc/](https://github.com/sanderwood/clamp3/tree/main/preprocessing/abc).
- **MIDI** files (.mtf) are preprocessed into MTF format using  [preprocessing/midi/batch_midi2mtf.py](https://github.com/sanderwood/clamp3/tree/main/preprocessing/midi/batch_midi2mtf.py).
- **Audio features** (.npy), extracted using MERT, are processed using [preprocessing/audio/extract_mert.py](https://github.com/sanderwood/clamp3/blob/main/preprocessing/audio/extract_mert.py).
...
### 3. [extract_m3.py](https://github.com/sanderwood/clamp3/blob/main/code/extract_m3.py)
...
Unlike [extract_clamp3.py](https://github.com/sanderwood/clamp3/blob/main/code/extract_clamp3.py), which aligns text and music for global semantic vector extraction, this script does not align with text and retains only temporal information. Each feature corresponds to a patch (a bar in ABC notation or a MIDI message).
...
### 4. [train_clamp3_audio.py](https://github.com/sanderwood/clamp3/blob/main/code/train_clamp3_audio.py) and [train_clamp3_symbolic.py](https://github.com/sanderwood/clamp3/blob/main/code/train_clamp3_symbolic.py)
...
- [train_clamp3_audio.py](https://github.com/sanderwood/clamp3/blob/main/code/train_clamp3_audio.py) for MERT-extracted audio features (.npy).
- [train_clamp3_symbolic.py](https://github.com/sanderwood/clamp3/blob/main/code/train_clamp3_symbolic.py) for symbolic music data (.abc, .mtf).
...
Training data paths are defined in [config.py](https://github.com/sanderwood/clamp3/blob/main/code/config.py) under `TRAIN_JSONL`, and validation uses `EVAL_JSONL` by default.
...
### 5. [train_m3.py](https://github.com/sanderwood/clamp3/blob/main/code/train_m3.py)
...
### 6. [utils.py](https://github.com/sanderwood/clamp3/blob/main/code/utils.py)
